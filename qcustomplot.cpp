  ...
  if (mAutoTickLabels)
  {
    ...
    if (mTickLabelType == ltNumber)
    {
      for (int i=mLowestVisibleTick; i<=mHighestVisibleTick; ++i) {
        ...
        if (mTickVector.at(i)<0) mTickVectorLabels[i].insert(0,"-");
     }
...
{
  mColorBuffer.fill(qRgb(255, 255, 255), mLevelCount);
  loadPreset(preset);
}
...
void QCPColorGradient::updateColorBuffer()
{
  ...
  //mColorBuffer[0]=qRgb(255,255,255);
